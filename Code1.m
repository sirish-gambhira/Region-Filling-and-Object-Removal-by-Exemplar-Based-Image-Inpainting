...
clc;
close all;
image_name = 'river.jpg';
I_colour = imread(image_name);
% 
% I_colour = uint8(zeros(400,400,3));
% I_colour(1:200,1:200,1) = 255;
% I_colour(201:400,1:200,2) = 255;
% I_colour(1:200,201:400,3) = 255;
...
% image_name = 'tower_mask.jpg';
% binaryImage = rgb2gray(imread(image_name));
% binaryImage = double(binaryImage/255);
% 
...
while(check(binaryImage))
    ...
    [Cont_C,Cont_D,gmag] = get_priorities(Inew,xc,yc,binaryImage,C,slopex,slopey);
    [s,t,cmax] = get_max(Cont_C,Cont_D,xc,yc);
    [p,q] = get_sample(Inew_colour,source,binaryImage,s,t,gmag);
...
%%
function [Cont_C,Cont_D,gmag] = get_priorities(Inew,xc,yc,binaryImage,C,slopex,slopey)
len = size(xc,2);
...
function [p,q] = get_sample(Inew_colour,source,bImage,s,t,gmag)
   	[m, n,~] = size(Inew_colour);
%     [m,n] = size(Inew_colour);
    min_dist = Inf;
    grad = Inf;
